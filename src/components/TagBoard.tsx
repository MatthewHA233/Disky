...
import { Plus, X, Trash2, Settings, FolderOpen, File, Pencil, Check } from "lucide-react";
import { cn } from "../lib/utils";
import type { Tag, TaggedPath } from "../types";
  ...
  return (
    <div className="flex flex-col h-full bg-[#0D0D12] rounded-2xl border border-[#2A2A35] overflow-hidden shadow-2xl">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-[#13131A] border-b border-[#2A2A35] shrink-0">
        <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-[#888899] flex-1">
          标签看板
        </span>
        <button
          className={cn(
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden custom-scrollbar">
        <div className="flex h-full min-w-max">
          {columns.map(({ tag, items }) => (
            <div
              key={tag.id}
              className="flex flex-col w-[220px] min-w-[220px] border-r border-[#2A2A35] last:border-r-0"
            >
              {/* Column header */}
              <div
              ...
              >
                <span
                  ...
                  style={{ backgroundColor: tag.color }}
                />
                <span className="text-[11px] font-semibold text-[#FAF8F5] truncate">
                  {tag.name}
                </span>
                <span className="text-[10px] text-[#555] ml-auto">{items.length}</span>
              </div>

              {/* Column items */}
              <div className="flex-1 overflow-y-auto py-1 custom-scrollbar">
                {items.length === 0 ? (
                  <div className="px-3 py-4 text-center text-[10px] text-[#555]">
                    无标记文件
                  </div>
                ) : (
                  items.map((item) => (
                    <div
                      ...
                      className="group px-2 py-1.5 mx-1 my-0.5 rounded-lg hover:bg-[#FFFFFF]/5 cursor-pointer transition-colors relative"
                      onClick={() => onNavigateToPath(item.path)}
                    >
                      <div className="flex items-center gap-1.5">
                        {item.path.endsWith("\\") || item.path.includes("\\") ? (
                          <FolderOpen className="w-3 h-3 text-[#C9A84C] shrink-0" />
                        ) : (
                          <File className="w-3 h-3 text-[#EAE6DF]/70 shrink-0" />
                        )}
                        <span className="text-[11px] text-[#EAE6DF] truncate font-medium">
                          {item.name}
                        </span>
                      </div>
                      <div className="text-[10px] text-[#555] truncate mt-0.5 pl-[18px]">
                        {item.path}
                      </div>
                      <button
                        className="absolute right-1 top-1/2 -translate-y-1/2 p-1 rounded text-[#555] hover:text-[#E74C3C] opacity-0 group-hover:opacity-100 transition-all hover:bg-[#E74C3C]/10"
                        onClick={(e) => {
                          e.stopPropagation();
                          onRemoveTag(item.path, item.tag_id);
                        }}
                        title="移除标签"
                      >
